...
- `protected_parameter_patterns`: regexes requiring an exact double unlock in tune mode.
...
1. Build a draft session with `auto` identity values.
...
4. Run `scripts/discover_algorithm_profile.py` with the draft, log, and config.
...
9. Validate the registry, session contract, parser, resume checks, and a representative smoke run.
