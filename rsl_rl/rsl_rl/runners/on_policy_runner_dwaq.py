...
class OnPolicyRunnerDwaq:
    """针对 DWAQ 优化的 On-policy 运行器。
    适配 Policy 组自带历史记录 (flatten_history_dim=False) 的 3D 观测架构。
    """
    ...
    def learn(self, num_learning_iterations: int, init_at_random_ep_len: bool = False):  # noqa: C901
        ...
        obs = self.env.get_observations().to(self.device)
         # DWAQ 核心：初始化“上一时刻特权观测”
        # 在第一步，我们用当前时刻的 critic 填充作为占位符
        ...
        for it in range(start_iter, tot_iter):
            ...
            with torch.inference_mode():
                for _ in range(self.num_steps_per_env):
                    # DWAQ 执行动作：传入当前的 obs 和上一时刻的特权观测
                    actions = self.alg.act(obs, prev_critic_obs)
                     # 物理步进前，备份当前的 critic obs 作为下一步的 "prev"
                    # 此时的 obs["critic"] 对应时刻 t
                    ...
                    if len(new_ids) > 0:
                        prev_critic_obs[new_ids] = obs["critic"][new_ids]
    ...
    def _construct_algorithm(self, obs) -> DWAQPPO:
        """构建 DWAQ 算法实例。"""
        ...
        print(f"[OnPolicyRunnerDwaq] History Length: {history_length}")
        print(f"[OnPolicyRunnerDwaq] Derived Single Obs Dim: {single_obs_dim}")
        
        # 2. 初始化模型 (过滤掉 class_name)
        ...
        # 3. 初始化算法 (过滤掉 class_name 和 obs_dim)
        alg_params = self.alg_cfg.copy()
        alg_params.pop("class_name", None)
        alg_params.pop("obs_dim", None) # 避免重复传递
        
        alg = DWAQPPO(
            actor_critic, 
            device=self.device, 
            obs_dim=single_obs_dim, # 始终以环境真实维度为准
            multi_gpu_cfg=self.multi_gpu_cfg,
            **alg_params
        )

        # 4. 初始化存储器
        # 确保传入的 actions_shape 是个列表，符合 constructor 要求
        alg.init_storage(
            "rl",
            self.env.num_envs,
