...
from robot_lab.utils.wrappers.rsl_rl.rl_amp_dwaq_cfg import (
    ...
    RslRlActorCriticAmpDwaqCfg,
    RslRlAlgorithmAmpDwaqCfg
)

@configclass
class LWRoughAmpDwaqRunnerCfg(RslRlOnPolicyRunnerAmpDwaqCfg):
    num_steps_per_env = 24
    max_iterations = 100000
    save_interval = 1000
    experiment_name = "LW_leg_rough_amp_dwaq"
    obs_groups = {
        "policy": ["policy"],       
        "critic": ["critic"],
        "amp": ["amp"]    
    }
    policy = RslRlActorCriticAmpDwaqCfg(
        init_noise_std=1.0,
    ...
    algorithm = RslRlAlgorithmAmpDwaqCfg(
        value_loss_coef=1.0,
